...
import { GENERATE_TIMEOUT_MS } from '../server/api-handlers/generate-path.js';
import { INTERPRET_TIMEOUT_MS } from '../server/api-handlers/interpret-goal.js';
import { TRANSCRIBE_TIMEOUT_MS } from '../server/api-handlers/transcribe-voice.js';
...
test('interpretation timeout hierarchy is server < browser < Vercel max duration', () => {
  const maxDurationMs = vercel.functions['api/ai.js'].maxDuration * 1000;
...
test('transcription timeout hierarchy is server < browser < Vercel max duration', () => {
  const maxDurationMs = vercel.functions['api/voice.js'].maxDuration * 1000;
