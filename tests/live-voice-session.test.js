...
function installLiveMocks({ socketOpens = true } = {}){
  const original = {
...
test('live voice opens socket before recorder starts, streams chunks, and keeps Stop stable', async () => {
  ...
  try{
    await startLiveVoiceSession({
      ...
      requestTokenGrant:async () => ({ accessToken:'temporary-token', expiresIn:30 }),
    ...
    assert.equal(mock.sockets[0].protocols[0], 'token');
    assert.equal(mock.sockets[0].protocols[1], 'temporary-token');
