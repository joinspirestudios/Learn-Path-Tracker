...
export function discoveryControlsHTML(paths, state){
  ...
  return '<div class="aurora-discovery-search-header aurora-discovery-toolbar" aria-label="Discover public paths search and filters">'
    ...
    + '<button class="aurora-button aurora-button-ghost aurora-clear-control" id="clearDiscoverySearch" type="button" ' + (state.query ? '' : 'disabled') + '>Clear</button>'
    + '<button class="aurora-button aurora-button-ghost aurora-clear-control" id="clearDiscoveryFilters" type="button" ' + (isDiscoveryDefault(state) ? 'disabled' : '') + '>Reset filters</button>'
    + '</div>'
