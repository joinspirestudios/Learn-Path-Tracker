...
function agendaHTML({ dayNumber, date, tasks, dayLog, evidenceSubmissions }){
  ...
  return '<section class="daily-session-panel daily-agenda" aria-labelledby="dailyAgendaTitle">'
    ...
    + '<div class="detail-progress">' + esc(summary.requiredResolved) + '/' + esc(summary.requiredTotal || summary.total) + ' required</div></div>'
    ...
    + '</div>'
    + '<div class="daily-progress-row"><progress value="' + esc(summary.percent) + '" max="100" aria-label="Required task progress"></progress><span>' + esc(summary.percent) + '%</span></div>'
    + '<p class="hint">' + esc(encouragementForProgress(summary.percent)) + '</p>'
...
function taskCardHTML({ item, position, total, progress, dayNumber, evidenceForm, evidenceBusy }){
  ...
  return '<section class="daily-session-panel daily-task-card" aria-labelledby="dailyTaskTitle">'
    + '<div class="daily-session-topline"><span>Task ' + esc(position) + ' of ' + esc(total) + '</span><b>' + esc(progress.percent) + '%</b></div>'
    + '<div class="daily-progress-row"><progress value="' + esc(progress.percent) + '" max="100" aria-label="Required task progress"></progress><span>' + esc(progress.requiredResolved) + '/' + esc(progress.requiredTotal || progress.total) + '</span></div>'
...
function summaryHTML({ tasks, dayLog, evidenceSubmissions, complete = false }){
  ...
  const title = complete ? 'Day complete' : (canCompleteDailySession(tasks, dayLog, evidenceSubmissions) ? 'Everything required is documented' : 'Today so far');
  return '<section class="daily-session-panel daily-session-summary" aria-labelledby="dailySessionSummaryTitle">'
    ...
    + '<div><span>Evidence</span><b>' + esc((Array.isArray(evidenceSubmissions) ? evidenceSubmissions : []).length) + '</b></div>'
    + '</div>'
    ...
    + '</div>'
    + '<div class="daily-session-actions">'
    + (!complete && canCompleteDailySession(tasks, dayLog, evidenceSubmissions) ? '<button class="btn gold" id="completeDay" type="button" data-day="' + esc(dayLog.dayNumber || 1) + '">Complete Day ' + esc(dayLog.dayNumber || 1) + '</button>' : '')
    + (!complete && !canCompleteDailySession(tasks, dayLog, evidenceSubmissions) ? '<button class="btn gold daily-session-action" type="button" data-session-action="finish-pending">Finish pending tasks</button>' : '')
...
export function dailySessionHTML({
  ...
  const session = deriveDailySessionState({ pathId, dayNumber, tasks, dayLog, evidenceSubmissions, saveState, error });
  let body = '';
  if(session.phase === 'agenda') body = agendaHTML({ dayNumber, date, tasks, dayLog, evidenceSubmissions });
  else if(session.phase === 'evidence-preparation') body = evidencePrepHTML(evidenceSummary(tasks));
  else if(session.phase === 'complete') body = summaryHTML({ tasks, dayLog, evidenceSubmissions, complete:true });
  else if(session.phase === 'partial-summary' || session.phase === 'completion-check') body = summaryHTML({ tasks, dayLog, evidenceSubmissions });
  else {
    const states = sessionTaskStates(tasks, dayLog, evidenceSubmissions);
